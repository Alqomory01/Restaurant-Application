...
- **Access is Manager-only**, not just shown-then-locked. Kitchen Staff and Head Chef have no legitimate reason to see procurement/supplier data — the "Store" nav section doesn't render for them at all (`components/Shell.tsx`), and a direct/bookmarked `/store/...` URL still hits a real gate (`components/StoreAccessGate.tsx`) that shows a restricted-access screen instead of the page. This is a deliberate departure from Kitchen's own Costing/Reports pattern (nav visible to everyone, content locked per-role) — Store is a genuinely separate department in the underlying business, not just a Kitchen feature with a paywall.
...
- **Purchase orders** — multi-line create form (supplier, priority, delivery details, line items with running total), status filter chips, and a detail view per PO. Orders over ₦50,000 require Manager approval before being marked sent — under that, they're auto-sent. Approving is one click from the list; **rejecting requires a reason** (captured inline in the detail view, shown back on the PO afterward) so a store keeper knows what to fix before resubmitting — matches the spec's "manager rejects with reason" rule, not just a status flip.
...
**This is real interactive state, not static mockup data** — everything created or edited (suppliers, items, POs, GRNs) lives in a React Context (`lib/foodops/FoodOpsContext.tsx`) seeded from `lib/foodops/mockData.ts`, so the whole loop is genuinely testable end-to-end (approve a PO, receive it, watch the item's on-hand quantity actually change) within a session. It resets on page reload — there's no backend/database behind it yet, by design, since this pass was scoped frontend-only. Turning this into the real thing means Django models mirroring `lib/foodops/types.ts`, real endpoints, a proper RBAC role for Store Keeper vs. this Manager-only stand-in (see the full ERP spec's 10-role model), and wiring the Kitchen side's stock requests to actually reach this module's dispatch flow (Dispatch and Wastage screens from the reference mockup aren't built yet either — this pass covered the core procure-to-stock loop only).

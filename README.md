...
| `manager` | Manager | Everything in both Kitchen and Store, including full food-cost figures, margins, and PO approval. Also a Django superuser (`/admin` access). |

Kitchen and Store are modeled as separate departments, not a single ladder of access — Kitchen roles can't see Store's supplier/procurement data, and Store Keeper can't see Kitchen's production data, each enforced twice (nav items hidden outright, and a real gate blocking a direct/bookmarked URL too — not just a locked-looking page).
...
- **The Store module has no backend.** Everything under `/store/*` is a real, interactive frontend on in-memory mock state (`lib/foodops/`) — it doesn't persist across a reload. All 9 screens from the reference mockup are now built (Dashboard, Suppliers, Item master, Purchase orders, Receiving, Stock levels, Dispatch, Wastage, Reports); Dispatch is the one screen that already talks to a real backend (Kitchen's `stock-requests` endpoints), since matching Store items to Kitchen ingredients is currently name-based (case-insensitive) rather than a shared item master — a real shared master is worth revisiting once Store has its own backend. DineFlow (POS) and the Management module don't exist yet — see `mise_system_flows.html` for the eventual full-system design.
